...
interface Auction {
  ...
  tokenMintAddress: string;
  auctionType: 'standard' | 'dutch'; // New field for auction type
  bids: { bidder: string; amount: number; timestamp: string }[]; // New field for bid history/leaderboard
}
...
const AuctionPage = () => {
  ...
  const [backendWalletPublicKey, setBackendWalletPublicKey] = useState<PublicKey | null>(null); // New state for backend wallet public key
  ...
  const fetchAuctions = useCallback(async () => {
    try {
      ...
      if (Array.isArray(response.data)) {
        ...
        console.warn('API response for auctions is not an array:', response.data);
        setAuctions([]);
      }
    } catch (err) {
  ...
  // Fetch backend wallet public key on component mount
  useEffect(() => {
  ...
  }, [fetchAuctions, socket, toast]); // Added toast to dependency array
  ...
  const openBidModal = (auction: Auction) => {
    ...
    setBidAmounts((prev) => ({
      ...prev,
      [auction.auctionId]: prev[auction.auctionId] || auction.currentBidSOL + 0.01,
    }));
  };
  ...
  const handleBidAmountChange = (value: string) => {
    if (selectedAuction) {
      setBidAmounts((prev) => ({
        ...prev,
        [selectedAuction.auctionId]: parseFloat(value),
      }));
    }
  };
  ...
  const handlePlaceBid = async (auction: Auction) => {
    ...
    const bidAmount = bidAmounts[auction.auctionId];
    if (!bidAmount || bidAmount <= auction.currentBidSOL) {
      toast({
        title: 'Invalid Bid',
        description: `Your bid must be higher than the current bid of ${auction.currentBidSOL} SOL.`,
        variant: 'destructive',
      });
      return;
    }
  ...
  return (
    <div className="container mx-auto py-8 relative">
      <BackButton /> {/* Add BackButton */}
      <h1 className="text-4xl font-bold mb-8 text-center gradient-text">Live Property Auctions</h1>
      {auctions.length === 0 ? (
        <p className="text-center text-muted-foreground">No active auctions found at the moment.</p>
      ) : (
            ...
            <Card key={auction.auctionId} className="glass-card flex flex-col">
              ...
              <CardContent className="flex-grow">
                ...
                <div className="flex items-center space-x-2 mt-4">
                  <Button onClick={() => openBidModal(auction)} className="gradient-button w-full">
                    Place Bid
                  </Button>
                </div>
                ...
                <div className="mt-6 pt-4 border-t border-muted-foreground/20">
                  ...
                  {auction.bids && auction.bids.length > 0 ? (
                    <div className="space-y-1">
                      {auction.bids
                        .sort((a, b) => b.amount - a.amount) // Sort bids by amount descending
                        .map((bid, index) => (
      ...
      {selectedAuction && (
        <Dialog open={isBidModalOpen} onOpenChange={setIsBidModalOpen}>
          <DialogContent className="sm:max-w-[425px] glass-card">
            <DialogHeader>
              <DialogTitle>Place Bid on {selectedAuction.propertyId.title}</DialogTitle>
              <DialogDescription>
                Current Bid:{' '}
                <span className="font-bold text-green-500">{selectedAuction.currentBidSOL} SOL</span>
              </DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input
                  id="bidAmount"
                  type="number"
                  value={bidAmounts[selectedAuction.auctionId] || ''}
                  onChange={(e) => handleBidAmountChange(e.target.value)}
                  min={selectedAuction.currentBidSOL + 0.01}
                  step="0.01"
            ...
            <DialogFooter>
              ...
              <Button onClick={() => handlePlaceBid(selectedAuction)} className="gradient-button">
                Confirm Bid
              </Button>
            </DialogFooter>
          </DialogContent>
