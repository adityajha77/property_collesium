  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      try {
        setLoading(true);
        setError(null); // Clear any previous error
        const response = await fetch(`${import.meta.env.VITE_BACKEND_API_URL}/api/properties/transactions/user/${publicKey!.toBase58()}`); // publicKey is guaranteed to be not null here
        if (!response.ok) {
